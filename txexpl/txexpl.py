...
SAFE_PATTERNS = [
    (
        "matic",
        (
            "https://safe-client.safe.global/v1/chains/137/transactions/",
            "matic",
        ),
    ),
    (
        "eth",
        (
            "https://safe-client.safe.global/v1/chains/1/transactions/",
            "eth",
        ),
    ),
    ("bnb", ("https://safe-client.safe.global/v1/chains/56/transactions/", "bsc")),
    ("arb1", ("https://safe-client.safe.global/v1/chains/42161/transactions/", "arb")),
    ("avax", ("https://safe-client.safe.global/v1/chains/43114/transactions/", "avax")),
    ("oeth", ("https://safe-client.safe.global/v1/chains/10/transactions/", "op")),
    (
        "mantle",
...
def _match_pattern(text, pattern_list):
    for pattern, chain in pattern_list:
        if pattern in text:
            return chain
    return None
...
class TxExplainer(ExtProcessor):
    ...
    def _init_patterns(self, path, patterns):
        ...
        logger.info(f"Load pattern {path}")
    ...
    def _process_addr(self, addr):
        addr = self.peth.web3.to_checksum_address(addr)
    ...
    def gen_full_md_from_call(self, to, data, value=0):
        data = HexBytes(data)
        if len(data) < 4:
            return f"ETH Transfer, to: {to}: data:{data.hex()}, value:{value}"
    ...
    def _gen_from_scan_url(self, url):
        r = re.findall(r"[0-9a-fA-F]{64}", url)
        if not r:
            return None

        txid = r[0]
        chain = _match_pattern(url, SCAN_PATTERNS)
        if chain is None:
            return None
    ...
    def _gen_from_safe_url(self, url):
        ...
        safe_txid = r[0]
        tx_url, chain = _match_pattern(url, SAFE_PATTERNS)
        if chain is None:
            return None

        r = requests.get(f"{tx_url}{safe_txid}").json()
        txid = r["txHash"]
        if txid:
            self.switch_chain(chain)
            return self.gen_full_md_from_txid(txid)

        to = r["txData"]["to"]["value"]
        data = r["txData"]["hexData"]
        value = r["txData"]["value"]
